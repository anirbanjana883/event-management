    ...
    <>
      <nav
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ease-in-out border-b ${
          isScrolled
            ? "bg-[#030303]/80 backdrop-blur-xl border-white/10 py-2 shadow-[0_10px_30px_-10px_rgba(0,0,0,0.5)]" // Changed py-3 to py-2
            : "bg-black/20 backdrop-blur-sm border-transparent py-2" // Changed py-5 to py-3
        }`}
      >
        <div className="container mx-auto px-6">
          <div className="flex justify-between items-center relative">
            ...
            >
              ...
              <div className="relative w-8 h-8 bg-gradient-to-br from-red-600 to-red-900 rounded-lg flex items-center justify-center transform group-hover:rotate-6 transition-all duration-300 shadow-lg shadow-red-900/30 border border-white/10">
                <Ticket className="text-white drop-shadow-md" size={16} />{" "}
                {/* Reduce icon size to 16 */}
              </div>
          ...
          {user ? (
            <>
              <div className="flex items-center gap-4 mb-8 p-5 bg-white/5 rounded-2xl border border-white/10 backdrop-blur-md">
                ...
                <div>
                  ...
                  <p className="text-sm text-red-400 uppercase tracking-wider font-semibold">
                    {user.role}
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-4">
                {user.role === "organizer" && (
                  <Link
                    ...
                    className="mobile-link text-red-400 border border-red-500/20 bg-red-500/10"
                  >
                ...
                <Link
                  to="/dashboard"
                  onClick={() => setIsOpen(false)}
                  className="mobile-link text-white hover:bg-white/5"
                >
                  <LayoutDashboard /> Dashboard
                </Link>
                <button
                  onClick={handleLogout}
                  className="mobile-link text-gray-400 hover:text-red-400 hover:bg-white/5"
                >
                  <LogOut /> Log Out
                </button>
              </div>
            ...
            <div className="space-y-6">
              <Link
                to="/login"
                onClick={() => setIsOpen(false)}
                className="block text-4xl font-black text-transparent bg-clip-text bg-gradient-to-r from-gray-400 to-white hover:to-gray-400 transition-all"
              >
                Log In
              </Link>
              <Link
                to="/register"
                onClick={() => setIsOpen(false)}
                className="block text-4xl font-black text-transparent bg-clip-text bg-gradient-to-r from-red-500 to-orange-500 hover:from-red-400 transition-all"
              >
                Get Started
              </Link>
            </div>
